...
class StableDiffusionPipe():
    ...
    def Inpaint(self, num_images: int = 1, save: bool = True, use_limited_mem: bool = True):
        ...
        #path = "./stable-diffusion-v1-4" if self.use_local_model else "mad3310/stable-diffusion-fashion-v1-1"
        #path = "./stable-diffusion-v1-4" if self.use_local_model else "runwayml/stable-diffusion-inpainting"
        path = "./stable-diffusion-v1-4" if self.use_local_model else "stabilityai/stable-diffusion-2-1"
        #path = "./stable-diffusion-v1-4" if self.use_local_model else "CompVis/stable-diffusion-v1-4"
        ...
        # Load the model
        print("\nLoading model...")
        pipe = StableDiffusionInpaintPipeline.from_pretrained(path, use_auth_token="",
                                    local_files_only=local_files_only,
                                    torch_dtype=torch.float16, revision='fp16')
        pipe = pipe.to(self.device)
        print("\nModel loaded successfully")
        ...
        #inputs
        image_path_dir = "/home/adeeshb/Adeesh/VLR/project/VLR_Project_Stable_Diffusion/Code/Stable-Diffusion-Playground/input_images/original_sample/" 
        for input_image in os.listdir(image_path_dir):
            if (input_image.endswith('.jpg') or input_image.endswith('.png')):
                image_path = image_path_dir + input_image
                mask_path= "/home/adeeshb/Adeesh/VLR/project/VLR_Project_Stable_Diffusion/Code/Stable-Diffusion-Playground/input_masks/original_mask/fashion_sample1.jpg"
                prompt = "Complete Womans dress in high quality"
                strength = 0.6
                if not 0 <= strength <= 1:
                    raise ValueError("{} is an invalid strength value. Enter strength in [0, 1] range.".format(strength))
                ...
                # Save images
                ref = image_path.split("/")[-1].split(".")[0]
                if save:
                    print("Saving images...")
                    save_images(images,ref)
